...
class RemoteDeliver implements FileCallable<Integer> {
	...
	/* Common values */
	String component, String loadModule, String baseline, String jobName, String buildNumber, UCMDeliver ucmDeliver, Logger logger, Pipe pipe ) {
	...
	public Integer invoke( File workspace, VirtualChannel channel ) throws IOException {
		...
		/* Get version number from project+component */
			...
			if( ucmDeliver.buildnumberMajor.length() > 0 && ucmDeliver.buildnumberMinor.length() > 0 && ucmDeliver.buildnumberPatch.length() > 0 ) {
				...
				String error = ( ucmDeliver.buildnumberMajor.length() == 0 ? "Major missing. " : "" ) + ( ucmDeliver.buildnumberMinor.length() == 0 ? "Minor missing. " : "" ) + ( ucmDeliver.buildnumberPatch.length() == 0 ? "Patch missing. " : "" );
		...
		try {
			...
			if( stream.isReadOnly() ) {
				...
				// baseline.deliver( null, target, view.GetViewRoot(), null,
				// true, true, true );
		...
		if( UCMView.viewExists( viewtag ) ) {
			...
			try {
				...
				try {
					...
					if( ucmEe.stdout != null ) {
						hudsonOut.println( ucmEe.stdout );
					}
