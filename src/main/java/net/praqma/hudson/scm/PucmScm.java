...
import net.praqma.hudson.notifier.UCMDeliver;
...
public class PucmScm extends SCM {
	...
	public PucmScm(String buildProject, String component, String levelToPoll,
			String loadModule, boolean multiSite, boolean newest,
	...
	/**
	 * This methond handles all functionallity about creating the rigth workspace folder for the job..
	 */
	@Override
	public boolean checkout(AbstractBuild<?, ?> build, Launcher launcher,
		...
		logger.setLocalLog(new File(rdir + System.getProperty("file.separator")
				+ "log.log"));
		
		/* Make sure the cool library is also affected */
		Cool.setLogger(logger);

		logger.info(id + "PucmSCM checkout v. "
				+ net.praqma.hudson.Version.version);
		boolean result = true;

		PrintStream consoleOutput = listener.getLogger();
		consoleOutput.println("[PUCM] Praqmatic UCM v. "
				+ net.praqma.hudson.Version.version + " - SCM section started");
		...
		if (this.multiSite) {
			...
			state.setMultiSiteFrquency(((PucmScmDescriptor) getDescriptor())
					.getMultiSiteFrequencyAsInt() * 60000);
			logger.info(id + "Multi site frequency: "
					+ state.getMultiSiteFrquency());
		} else {
		...
		if(this.pollChild){
			/* Make deliver view */
			SnapshotView devView;
			SnapshotView intView;
			try
			{
				Stream devS = state.getBaseline().getStream();
				Stream intS = Stream.GetStream(this.stream);
				
				/* Replace evil characters with less evil characters */
				String newJobName = jobName.replaceAll( "\\s", "_" );
				String devViewTag = newJobName + "_" + System.getenv( "COMPUTERNAME" ) + "_" + devS.GetShortname();
				String intViewTag = newJobName + "_" + System.getenv( "COMPUTERNAME" ) + "_" + devS.GetShortname();
				
				devView = SnapshotView.Create(devS, new File (System.getenv("WORKSPACE"+"\\view")), devViewTag);
				
				intView = SnapshotView.Create(intS, new File (System.getenv("WORKSPACE"+"\\view")), intViewTag);
				
				state.getBaseline().deliver(devS, intS, intView.GetViewRoot(), intViewTag, true, false, true);
				
				

				//TODO BUILD in int view and if this goes well complete the deliver
				
				throw new ScmException("");
			}
			catch ( ScmException e )
			{
				logger.warning( id + "could not create deliver view: " + e.getMessage() );
				logger.warning( e );
		...
		if (build.getBuildVariables().get(baselinevalue) != null) {
			String baselinename = (String) build.getBuildVariables().get(
					baselinevalue);
			try {
				state.setBaseline(UCMEntity.GetBaseline(baselinename));
				state.setStream(state.getBaseline().getStream());
				consoleOutput
						.println("[PUCM] Starting parameterized build with a pucm_baseline.\n[PUCM] Using baseline: "
								+ baselinename
								+ " from integrationstream "
								+ state.getStream().GetShortname());
				...
				consoleOutput
						.println("[PUCM] Could not find baseline from parameter '"
								+ baselinename + "'.");
		...
		/* Default stream + component case */
			...
			if (!state.isAddedByPoller()) {
				try {
					List<Baseline> baselines = getValidBaselines(
							build.getProject(), state,
							Project.GetPlevelFromString(levelToPoll));
		...
		if (state.getBaseline() != null) {
			consoleOutput.println("[PUCM] building baseline "
					+ state.getBaseline());
			...
			try {
				...
				build.setDescription("<small>" + state.getBaseline()
						+ "</small>");
				CheckoutTask ct = new CheckoutTask(listener, jobName,
						build.getNumber(), state.getStream().GetFQName(),
						loadModule, state.getBaseline().GetFQName(),
						buildProject, logger);
				...
				try {
					...
					consoleOutput
							.println("[PUCM] Could not write change log file");
				}

			} catch (Exception e) {
				consoleOutput.println("[PUCM] An unknown error occured: "
						+ e.getMessage());
				logger.warning(e);
	...
	public PollingResult compareRemoteRevisionWith(
			...
			FilePath workspace, TaskListener listener, SCMRevisionState rstate)
			throws IOException, InterruptedException {
		/* Preparing the logger */
		logger = PraqmaLogger.getLogger();

		this.id = "[" + project.getDisplayName() + "::"
...
consoleOut.println("[PUCM] polling on child streams: "+pollChild);
		if (this.pollChild) {
			consoleOut
					.println("[PUCM] we wont poll on the integration stream insted we are looking for all new baselines on all stream that has the defined int stream as there default deliver target");
			try {
				// Finds all child stream this integration stream..
				List<Stream> cStreams = Stream.GetStream(this.stream).getChildStreams();
	...
	private PollingResult getPossibleBaselines(AbstractProject<?, ?> project,
			State state, TaskListener listener) {
		...
		try {
			baselines = getValidBaselines(project, state,
					Project.GetPlevelFromString(levelToPoll));
	...
	private List<Baseline> getValidBaselines(AbstractProject<?, ?> project,
			State state, Project.Plevel plevel) throws ScmException {
		...
		try {
			if (state.getStream() == null) {
				logger.print("[PUCM] Development " + state.getStream());
		...
		try {
			baselines = state.getComponent().GetBaselines(state.getStream(),
					plevel);
			logger.debug(id + "GetBaseline state:\n" + state.stringify());
		} catch (UCMException e) {
			throw new ScmException(
					"Could not retrieve baselines from repository. "
							+ e.getMessage());
		}
		logger.debug(id + "GetBaseline state:\n" + state.stringify());
		...
		if (baselines.size() > 0) {
			...
			if (state.isMultiSite()) {
				...
				int pruned = PucmScm.storedBaselines.prune(state
						.getMultiSiteFrquency());
				logger.info(id
						+ "I pruned "
						+ pruned
						+ " baselines from cache with threshold "
						+ StoredBaselines.milliToMinute(state
								.getMultiSiteFrquency()) + "m");
			...
			try {
				...
				for (Baseline b : baselines) {
					...
					State cstate = pucm.getStateByBaseline(jobName,
							b.GetFQName());
					...
					if (state.isMultiSite()) {
						...
						sbl = PucmScm.storedBaselines
								.getBaseline(b.GetFQName());
					...
					if (cstate != null) {
						...
						if (!bld.isLogUpdated()) {
							logger.debug(id + "Job " + bld.getNumber()
									+ " is not building");
							 ...
							 */
							if (sbl == null
									|| sbl.plevel == b.getPromotionLevel(true)) {
								logger.debug(id + b
										+ " was added to selected list");
								validBaselines.add(b);
							}
						} else {
							logger.debug(id + "Job " + bld.getNumber()
									+ " is building "
									+ cstate.getBaseline().GetFQName());
						}
					}
				...
				if (validBaselines.size() == 0) {
					logger.log(id
							+ "No baselines available on chosen parameters.");
					throw new ScmException(
							"No baselines available on chosen parameters.");
				}

			} catch (UCMException e) {
				throw new ScmException("Could not get recommended baselines. "
						+ e.getMessage());
			}
		} else {
